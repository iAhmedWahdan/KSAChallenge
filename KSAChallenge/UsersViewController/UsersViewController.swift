...
class UsersViewController: UIViewController {
    ...
    func reloadUsers() {
        self.viewModel.loadUsers(loadingMore: false, searchText: searchText)
    }
    ...
    private func setupSearchController() {
        ...
        searchController.searchBar.rx.text.orEmpty.distinctUntilChanged().bind { [weak self] text in
            guard let self = self else { return }
            self.viewModel.loadUsers(loadingMore: false, searchText: self.searchText)
        }.disposed(by: bag)
    }
...
extension UsersViewController {
    ...
    private func setupTableView() {
        ...
        tableView.rx.willDisplayCell.map {
            ...
            guard let self = self else { return }
            self.viewModel.loadUsers(loadingMore: true, searchText: self.searchText)
        }.disposed(by: bag)
